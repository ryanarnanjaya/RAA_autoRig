...
class CreateIK:
    ...
    def __init__(self, IK_name, jnt_A, jnt_B, jnt_C, ctrl_IK):
        ...
        self.rev_weight = None

        self.poc_hdl_IK = None
    ...
    def soft_IK(self):
        ...
        mdv_div_x = pm.createNode('multiplyDivide', name = 'mdv_' + self.IK_name + '_div_x')
        mdv_div_x.operation.set(2)
        cnd.outColor.outColorR >> mdv_div_x.input1.input1X
        dbw_x.distance >> mdv_div_x.input2.input2X
        ...
        mdv_div_x.output.outputX >> self.rev_weight.input.inputX

        if self.poc_hdl_IK is None:
            # if stretch_IK() has not been called
            ...
            mdv_div_x.output.outputX >> poc_hdl_IK_attr_W1

        else:
            # if stretch_IK() has been called
            ...
            # this way, stretch overrides soft
    ...
    def stretch_IK(self):
        ...
        if self.rev_weight is None:
            # if soft_IK() has not been not called
            self.poc_hdl_IK = pm.pointConstraint(self.jnt_A_scale, self.ctrl_IK, self.hdl_IK, maintainOffset = False)
            poc_hdl_IK_attr_W0 = self.poc_hdl_IK.attr(self.jnt_A_scale.name(long = None) + 'W0')
            poc_hdl_IK_attr_W1 = self.poc_hdl_IK.attr(self.ctrl_IK.name(long = None) + 'W1')
